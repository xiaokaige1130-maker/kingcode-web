...
function getSkillRoots(config) {
  ...
  return uniquePaths([
    ...
    ,
    workspaceRoot ? path.join(workspaceRoot, ".openclaw", "skills") : ""
...
function detectSourceType(rootPath, config) {
  ...
  if (workspaceRoot && resolvedRoot === path.join(workspaceRoot, ".codex", "skills")) {
    return "codex";
  }
  if (workspaceRoot && resolvedRoot === path.join(workspaceRoot, ".claude", "skills")) {
    return "claude";
  }
  if (workspaceRoot && resolvedRoot === path.join(workspaceRoot, ".openclaw", "skills")) {
    return "openclaw";
  }
  return "custom";
}
